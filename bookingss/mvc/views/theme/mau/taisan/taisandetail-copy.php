							...
							<tr class="text-center">
								...
								<td>
										...
										<div class="d-flex justify-content-center gap-2 flex-wrap">
											...
											<?php if ($value->status == 0): ?>
												...
												<button class="btn btn-primary btn-sm text-nowrap datphong"
													data-id="<?= $value->id ?>" data-status="2"
												...
												<button class="btn btn-danger btn-sm text-nowrap huyphong" style="display:none"
													data-id="<?= $value->id ?>" data-status="1" data-kt="<?= $_COOKIE['admin'] ?>">
													Hủy đặt
												</button>
									...
									<?php elseif (!isset($_COOKIE['admin'])):  ?>
										<a class="btn btn-info btn-sm text-nowrap ">
											Vui lòng đăng nhập
										</a>
...
<script>
	$(document).ready(function() {
		...
		$('.huyphong').on('click', function() {
			...
			const status_phong = $(this).data('status');
