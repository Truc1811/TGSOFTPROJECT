...
<div class="col-12 col-md-6 pe-0 pe-md-4 mb-3">
    ...
    <input type="hidden" name="image_m1" value="<?php echo $dataa[0]->image_m; ?>"  />
    <div class="d-flex flex-wrap hienthim">
        <?php  
            $mh=explode(',',$dataa[0]->image_m);
            if(count($mh)>0){
                foreach ($mh as $key => $value) {
                    if($value!='noimage' && $value!='file_khong_hop_le'){
            
        ?>
                <img class="pe-2 pb-2 phongto cursor" style="height:8vh" src="public/upload/<?php echo $value; ?>"/>
        <?php 
                    }
                }
            }
...
<div class="col-12 col-md-6 pe-0 pe-md-4 mb-3">
    ...
    <input type="text" name="ma_sp" class="form-control" id="ma_sp"
        value="<?php if(@$_POST['ma_sp']){ echo $_POST['ma_sp'];} else { echo $dataa[0]->ma_sp;} ?>"
    >
</div>
<div class="col-12 col-md-6 pe-0 pe-md-4 mb-3">
    <label for="quatang" class="form-label">Quà tặng kèm</label>
    <textarea id="quatang" name="quatang" class="form-control"><?php if(@$_POST['quatang']){ echo $_POST['quatang'];} else { echo $dataa[0]->quatang;} ?></textarea>
     <script>

        CKEDITOR.replace('quatang',{
...
<div class="col-12 col-md-6 pe-0 pe-md-4 mb-3">
    <label for="thongtinbosung" class="form-label">Thông tin bổ sung</label>
    <textarea id="thongtinbosung" name="thongtinbosung" class="form-control"><?php if(@$_POST['thongtinbosung']){ echo $_POST['thongtinbosung'];} else { echo $dataa[0]->thongtinbosung;} ?></textarea>
    <script>

        CKEDITOR.replace('thongtinbosung',{
                          // Define the toolbar groups as it is a more accessible solution.
                          toolbarGroups: [
                            //{ name: 'document', groups: [ 'mode', 'document', 'doctools' ] },
                            //{ name: 'clipboard', groups: [ 'clipboard', 'undo' ] },
